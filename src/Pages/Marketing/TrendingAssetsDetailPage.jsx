...
const TrendingAssetsDetailPage = () => {
  ...
  if (!asset) {
    return (
      <div className="min-h-screen flex items-center justify-center text-gray-600">No asset data found.</div>
    );
  }
    ...
    <div className="bg-gray-50 min-h-screen p-6 md:p-10">
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="flex items-baseline space-x-3">
                  ...
                  <p className={`font-medium text-sm ${isPositive ? "text-green-600" : "text-red-600"}`}>{asset.change}</p>
                </div>
              </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-6 mt-4">
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={chartData}>
                  ...
                  <Line type="monotone" dataKey="price" stroke={isPositive ? "#16a34a" : "#ef4444"} strokeWidth={2} dot={false} fill="url(#grad)" />
                </LineChart>
              </ResponsiveContainer>
            </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm self-start">
          ...
          <div className="flex mb-6 bg-gray-50 rounded-lg p-1 border border-gray-100">
            <button
              onClick={() => setIsBuy(true)}
              className={`flex-1 py-2 font-semibold rounded-md transition ${
                isBuy ? "bg-green-500 text-white shadow-sm" : "text-gray-600 hover:bg-gray-100"
              }`}
            >
              Buy
            </button>
            <button
              onClick={() => setIsBuy(false)}
              className={`flex-1 py-2 font-semibold rounded-md transition ${
                !isBuy ? "bg-red-500 text-white shadow-sm" : "text-gray-500 hover:bg-gray-100"
              }`}
            >
              Sell
            </button>
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <label className="inline-flex items-center cursor-pointer">
              ...
              <div className="w-11 h-6 bg-gray-200 rounded-full peer-checked:bg-blue-500 relative transition">
                <div className="absolute top-[2px] left-[2px] h-5 w-5 bg-white rounded-full transition-all peer-checked:translate-x-5 shadow-sm"></div>
              </div>
            </label>
          </div>
      ...
      <div className="max-w-7xl mx-auto mt-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white rounded-2xl shadow-md p-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            {[{ name: "Binance Coin", change: "-3.14%" },{ name: "Cardano", change: "-6.37%" },{ name: "Chainlink", change: "-7.41%" },{ name: "TRON", change: "-0.69%" },{ name: "Shiba Inu", change: "-3.43%" },{ name: "Dogecoin", change: "-4.90%" },{ name: "Ethereum", change: "-2.09%" },{ name: "Solana", change: "-4.81%" },{ name: "PEPE", change: "-12.25%" },{ name: "Avalanche", change: "-10.93%" }].map((asset, i) => (
              <div key={i} className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg p-3 hover:bg-gray-100 transition shadow-sm">
                <div className="flex items-center space-x-3">
                  <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center font-semibold text-blue-600">{asset.name.charAt(0)}</div>
                  <span className="text-sm font-medium text-gray-800">{asset.name}</span>
                </div>
                <span className="text-sm text-red-500 font-semibold">{asset.change}</span>
              </div>
            ))}
          </div>
        </div>
